...
export function createFilesRouter(configRepo: ConfigRepository) {
  const router = Router();
  const usersRepo = new UsersRepository((configRepo as unknown as { db: ConstructorParameters<typeof UsersRepository>[0] }).db);

  const getWorkspaceDir = async (req: Request): Promise<string | null> => {
  ...
  const withWorkspace = (
    handler: (req: Request, res: Response, workspaceDir: string) => Promise<void>,
  ...
  router.get('/', withWorkspace(async (req, res, workspaceDir) => {
    ...
    if (!targetStat.isDirectory()) throw apiError(400, 'Path must be a directory');
  ...
  router.get('/content', withWorkspace(async (req, res, workspaceDir) => {
    ...
    if (!fileStat.isFile()) throw apiError(400, 'Path must be a file');
    ...
    if (buffer.includes(0)) throw apiError(415, 'Binary files cannot be opened in the text editor');
  ...
  router.post('/upload', handleMultipart(upload.array('files', 10)), withWorkspace(async (req, res, workspaceDir) => {
    ...
    if (!files.length) throw apiError(400, 'No files uploaded');
    const totalBytes = files.reduce((total, file) => total + file.size, 0);
    if (totalBytes > MAX_UPLOAD_TOTAL_BYTES) throw apiError(413, 'Combined upload exceeds 100 MB');
  ...
  router.post('/mkdir', withWorkspace(async (req, res, workspaceDir) => {
    ...
    const fullPath = safeWorkspacePath(workspaceDir, requestedPath);
    if (fullPath === resolve(workspaceDir)) throw apiError(409, 'The workspace root already exists');
  ...
  router.delete('/', withWorkspace(async (req, res, workspaceDir) => {
    const requestedPath = requiredQuery(req, 'path');
    const fullPath = safeWorkspacePath(workspaceDir, requestedPath);
    if (fullPath === resolve(workspaceDir)) throw apiError(400, 'The workspace root cannot be deleted');
    await stat(fullPath);
  ...
  router.post('/rename', withWorkspace(async (req, res, workspaceDir) => {
    ...
    if (typeof oldPath !== 'string' || typeof newPath !== 'string') throw apiError(400, 'oldPath and newPath are required');
    const fullOld = safeWorkspacePath(workspaceDir, oldPath);
    const fullNew = safeWorkspacePath(workspaceDir, newPath);
    if (fullOld === resolve(workspaceDir) || fullNew === resolve(workspaceDir)) throw apiError(400, 'The workspace root cannot be renamed');
    await stat(fullOld);
    if (await pathExists(fullNew)) throw apiError(409, 'A file or folder already exists at the destination');
  ...
  router.post('/create-file', withWorkspace(async (req, res, workspaceDir) => {
    const requestedPath = requiredBodyPath(req);
    const fullPath = safeWorkspacePath(workspaceDir, requestedPath);
    if (fullPath === resolve(workspaceDir)) throw apiError(400, 'A file path is required');
    await mkdir(dirname(fullPath), { recursive: true });
  ...
  router.post('/extract-zip', handleMultipart(upload.single('zipfile')), withWorkspace(async (req, res, workspaceDir) => {
    const file = req.file;
    const destination = typeof req.body.destination === 'string' ? req.body.destination : '.';
    if (!file) throw apiError(400, 'No ZIP file uploaded');
    const safeDest = safeWorkspacePath(workspaceDir, destination);
    const zip = new AdmZip(file.buffer);
    const entries = zip.getEntries();
    if (entries.length > MAX_ZIP_ENTRIES) throw apiError(413, `ZIP contains more than ${MAX_ZIP_ENTRIES} entries`);

    let totalBytes = 0;
    const validated = entries.map(entry => {
      const entryName = validateZipEntry(entry.entryName);
      const entrySize = entry.header.size;
      if (entrySize > MAX_ZIP_ENTRY_BYTES) throw apiError(413, `ZIP entry is too large: ${entryName}`);
      totalBytes += entrySize;
      if (totalBytes > MAX_ZIP_TOTAL_BYTES) throw apiError(413, 'Expanded ZIP exceeds 200 MB');
    ...
    for (const item of validated) {
      if (item.entry.isDirectory) {
        ...
        if (data.length > MAX_ZIP_ENTRY_BYTES) throw apiError(413, `ZIP entry is too large: ${item.entryName}`);
...
async function listDir(
  ...
  if (depth > MAX_TREE_DEPTH) throw apiError(413, `Directory tree exceeds ${MAX_TREE_DEPTH} levels`);
  ...
  const results = await Promise.all(visible.map(async entry => {
    counter.count += 1;
    if (counter.count > MAX_TREE_ENTRIES) throw apiError(413, `Directory tree exceeds ${MAX_TREE_ENTRIES} entries`);
...
function requiredQuery(req: Request, name: string): string {
  ...
  if (!value) throw apiError(400, `${name} query parameter is required`);
  return value;
}
...
function requiredBodyPath(req: Request): string {
  const value = req.body?.path;
  if (typeof value !== 'string' || !value.trim()) throw apiError(400, 'path is required');
  return value.trim();
}
...
function sanitizeUploadName(name: string): string {
  ...
  if (!safe || safe === '.' || safe === '..') throw apiError(400, 'Invalid upload filename');
  return safe;
}

function validateZipEntry(name: string): string {
...
function isLikelyEditable(name: string): boolean {
...
function handleMultipart(middleware: ReturnType<typeof upload.array>) {
  return (req: Request, res: Response, next: () => void) => {
    middleware(req, res, error => {
      if (error) sendFileError(res, error);
      else next();
    });
  };
}
...
function sendFileError(res: Response, error: unknown): void {
  ...
  const status = error instanceof Error && 'status' in error ? Number((error as { status: number }).status) : statusForNodeError(error);
  const message = status >= 500 ? 'File operation failed' : error instanceof Error ? error.message : 'File operation failed';
  if (status >= 500) log.error('File operation failed', { error: error instanceof Error ? error.message : String(error) });
  res.status(status).json({ error: message });
}

function statusForNodeError(error: unknown): number {
  if (!isNodeError(error)) return 500;
  if (error.code === 'ENOENT') return 404;
  if (error.code === 'EEXIST' || error.code === 'ENOTEMPTY') return 409;
