...
const toolActionMap: Record<string, { label: string; icon: React.ComponentType<{ className?: string }> }> = {
  ...
  deleteFile: { label: 'Excluir arquivo', icon: Trash2 },
  runCommand: { label: 'Executar comando', icon: Terminal },
  executeCode: { label: 'Executar código', icon: Code },
  searchFiles: { label: 'Pesquisar arquivos', icon: Search },
  webFetch: { label: 'Buscar URL', icon: Globe },
  installPackage: { label: 'Instalar pacote', icon: Package },
  invokeSubAgent: { label: 'Delegar subtarefa', icon: Users },
};
...
function getSummary(toolName: string, input: unknown): string {
  ...
  switch (toolName) {
    ...
    case 'searchFiles':
      return String(obj.pattern || obj.query || 'pesquisa');
    case 'webFetch':
    ...
    case 'invokeSubAgent':
      return String(obj.task || obj.description || 'subtarefa');
    default:
    ...
    <Dialog open onOpenChange={() => onRespond(false)}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>O agente quer executar uma ação que exige sua aprovação</DialogDescription>
        </DialogHeader>
